...
import com.google.sha1coin.core.WrongNetworkException;
...
public final class PaymentIntent implements Parcelable
{
	...
	{
		final Output[] outputs = buildSimplePayTo(BigInteger.valueOf(bitcoinUri.getAmount().longValue()), bitcoinUri.getAddress());
	...
	public SendRequest toSendRequest()
	{
		...
		for (final PaymentIntent.Output output : outputs)
			transaction.addOutput(Coin.valueOf(output.amount.longValue()), output.script);
